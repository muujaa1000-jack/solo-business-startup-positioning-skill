...
_LOCAL_PROJECT = re.compile(r"(?i)[a-z]:\\codex\\")
...
def _text_files(root: Path) -> list[Path]:
    ...
    for path in root.rglob("*"):
        ...
        if any(part in _IGNORED_DIRECTORIES for part in relative_parts):
            continue
        if relative_parts[:2] == ("docs", "superpowers"):
            continue
        files.append(path)
    return files
...
def validate_repository(root: Path) -> list[str]:
    ...
    if metadata is not None and f"${SKILL_NAME}" not in metadata:
        findings.append("agents/openai.yaml: default prompt must invoke the Skill by name")
